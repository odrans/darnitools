...
#' @export
l2.extract <- function(fn, dir.rds = NULL, only.ice = TRUE, keep.index = FALSE) {
  ...
  tmp <- data.frame(expand.grid(idx.height=1:nheight,
                                idx.time=1:ntime),
                    plev=as.numeric(ncdf4::ncvar_get(nc,"plev")),
                    ...
                    iwc=as.numeric(ncdf4::ncvar_get(nc,"iwc"))
                    ) %>%
    ...
    dplyr::mutate(lat=ncdf4::ncvar_get(nc,"lat")[idx.time] %>% as.numeric(),
                  ...
                  time = as.POSIXct(ncdf4::ncvar_get(nc, "base_time"), tz = "UTC", origin = "1970-01-01") + as.numeric(ncdf4::ncvar_get(nc, "dtime")[idx.time]),
                  flag.iter=ncdf4::ncvar_get(nc,"iteration_flag")[idx.time] %>% as.numeric(),
                  flag.precip=ncdf4::ncvar_get(nc,"precipitation_flag")[idx.time] %>% as.numeric(),
                  nightday_flag=ncdf4::ncvar_get(nc,"nightday_flag")[idx.time] %>% as.numeric(),
                  tc=tc-273.15,
                  icnc_5um=icnc_5um*1E-3) %>%
    dplyr::filter(flag.iter==1) %>%
    dplyr::mutate(flag.ice= (icnc_5um > 0 & iwc > 1E-8 & clm == 1 & flag.mixed==0 & tc < 270),
                  icnc_5um=replace(icnc_5um,!flag.ice,0))

  if(only.ice) {tmp  <- tmp %>% dplyr::filter(flag.ice) %>% dplyr::select(-c(flag.ice))}

  if(keep.index) {
    tmp <- tmp %>% dplyr::select(-c(clm,flag.mixed,flag.iter))
  } else {
    tmp <- tmp %>% dplyr::select(-c(clm,flag.mixed,flag.iter,idx.height,idx.time))
  }
